...
Метод lower () — преобразует строку в нижний регистр, в случае, если в строке не существует символов верхнего регистра, то он возвращает исходную строку без изменений.
...
### Напишите программу, которая считывает целое число, после чего на экран выводится следующее и предыдущее целое число с пояснительным текстом. Программа должна занимать не более двух строк редактора кода.
...
В коде была использована F-строка (для включения значения переменных, которые вычисляются во время выполнения программы, в данном случае - для вычисления следующего и предыдущего числа для введеного с консоли целого числа). Для того, чтобы удостовериться, что ввод будет именно целочисленным, была использована функция int().
...
### Выведите предложение ‘Hello World’ в обратном порядке. Программа должна занимать не более двух строк редактора кода.
...
1. Срез (slice) — извлечение из данной строки одного символа или некоторого фрагмента (подстроки). Есть три формы срезов. Самая простая форма среза: взятие одного символа строки, а именно, S[i] — это срез, состоящий из одного символа, который имеет номер i, при этом считая, что нумерация начинается с числа 0. Если указать отрицательное значение индекса, то номер будет отсчитываться с конца, начиная с номера -1.
2. :: означает вывод всех символов в заданном порядке.
