...
"""Функция нахождения квадратного корня от суммы квадратов значений (чисел списка) ключа из словаря kwargs"""
    for key in kwargs.items(): # для ключа из элементов словаря kwargs
        result = sqrt(key[1][0] ** 2 + key[1][1] ** 2)
        """вычисляем значние result, которое является квадратным корнем от суммы квадратов двух чисел из списка. 
...
if __name__ == '__main__':
    start_time = datetime.now() # задаем переменную начала работы программы
